...
export async function markRateLimitCooldown(): Promise<void> {
  globals().__coingeckoRateLimitedUntil = Date.now() + COOLDOWN_MS;
  const sec = Math.round(COOLDOWN_MS / 1000);
  await logger.warn(
    `CoinGecko rate limit — no API calls for ${sec}s. Showing cached prices.`,
  );
}

export async function applyStaleCacheFallback(
  coins: MarketCoin[],
  reason: string,
): Promise<void> {
  if (coins.length === 0) {
  ...
  await logger.info(`Using cached prices (${coins.length} coins). ${reason}`);
}
